...
./analyze.go:38:9: undefined: cp_scan
...
/go/src/go/scanner/controller# go help install
...
root@457925f0a75b:/go/src/go/scanner/controller# go install cp_scan.go /go/src/gene/          
named files must be .go files

root@457925f0a75b:/go/src/go/scanner/controller# go install cp_scan.go /go/src/go/scanner/controller/cp_scan.go
named files must all be in one directory; have ./ and /go/src/go/scanner/controller/

root@457925f0a75b:/go/src# pwd
/go/src

root@457925f0a75b:/go/src# go install go/
can't load package: package go: no Go files in /usr/local/go/src/go
root@457925f0a75b:/go/src# go install go/
analyze.go  c.log.tgz   scanner/    untar.go    
root@457925f0a75b:/go/src# go install go/scanner/controller/


root@457925f0a75b:/go/src/go# go run analyze.go 
analyze.go:8:5: cannot find package "scanner/controller" in any of:
        /usr/local/go/src/scanner/controller (from $GOROOT)
        /go/src/scanner/controller (from $GOPATH)
...
import (
    ...
    "os"
	"strconv"
    "go/scanner/controller"
)
cp_scan.scan()
root@457925f0a75b:/go/src/go# go run analyze.go c.log.tgz 
# command-line-arguments
./analyze.go:8:5: imported and not used: "go/scanner/controller"
./analyze.go:38:9: undefined: cp_scan
...
import (
    ...
    "os"
	"strconv"
    "go/scanner/controller/cp_scan"
)
root@457925f0a75b:/go/src/go# go run analyze.go c.log.tgz 
analyze.go:8:5: cannot find package "go/scanner/controller/cp_scan" in any of:
        /usr/local/go/src/go/scanner/controller/cp_scan (from $GOROOT)
        /go/src/go/scanner/controller/cp_scan (from $GOPATH)

controller.scan()
root@457925f0a75b:/go/src/go# go run analyze.go c.log.tgz 
# command-line-arguments
./analyze.go:38:9: cannot refer to unexported name controller.scan
./analyze.go:38:9: undefined: controller.scan

https://www.sneppets.com/golang/cannot-refer-unexported-name-undefined-error-go/
https://motion-express.com/blog/organizing-a-go-project
...
In contrast to the semantics for bean methods in @Configuration classes, 'inter-bean references' are not supported in lite mode. Instead, when one @Bean-method invokes another @Bean-method in lite mode, the invocation is a standard Java method invocation; Spring does not intercept the invocation via a CGLIB proxy. This is analogous to inter-@Transactional method calls where in proxy mode, Spring does not intercept the invocation ¡ª Spring does so only in AspectJ mode.
For example:
   @Component
